...
export function SummaryPage() {
  ...
  const aiSummary = posts.slice(0, 3).map((post) => post.title);
  const doctorPacket = comments.slice(0, 3).map((comment) => comment.body);
  const recommendations = comments.slice(3, 6).map((comment) => comment.body);
